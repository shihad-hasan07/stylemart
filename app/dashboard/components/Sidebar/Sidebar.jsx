...
export default function Sidebar({ role }) {
    ...
    return (
        <div className="shrink-0">
            ...
            {/* display block overlay in small device */}
            {isMobileOpen && (<div onClick={toggleMobileSidebar} className="fixed cursor-pointer top-20 left-0 right-0 bottom-0 bg-black/40 z-40 lg:hidden" />)}
            ...
            >
                ...
                <div className="flex items-center px-3 pt-4 h-[70px]">
                    {/* owner */}
                    {
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <p className={` w-44 text-xl font-medium transition-all ease-out
                                ${shouldExpand ? 'opacity-100 translate-x-2 duration-300 delay-75' : 'opacity-0 -translate-x-4 pointer-events-none duration-100'}`}>
                                Store Administrator
                            </p>
                        </div>
                    }
                    ...
                    {/* manager */}
                    {
                        ...
                        <div className="flex items-center gap-2.5">
                            <div className={` w-9 shrink-0 h-9 bg-[#3a2f5a] rounded-full transition-all duration-300
                                ${shouldExpand ? 'translate-x-2' : 'translate-x-2'}`}>
                                <img src="/dashboard/manager.png" className="w-full h-full rounded-xl object-cover" alt="manager" />
                            </div>
                            <p className={` w-40 text-xl font-medium transition-all ease-out
                                ${shouldExpand ? 'opacity-100 translate-x-2 duration-300 delay-75' : 'opacity-0 -translate-x-2 pointer-events-none duration-100'}`}>
                                Store Manager
                            </p>
                        </div>
                    }
                ...
                <Link href='/' onClick={() => {
                    ...
                    <div className={`flex -mt-3.5 hover:bg-gray-100 rounded-xl py-2.5 px-1 items-center mx-3 transition-all duration-300 ${shouldExpand ? 'opacity-100 -translate-y-3 h-fit' : 'opacity-0 -translate-y-3 h-0 overflow-hidden pointer-events-none'}`}>
                        <BsArrowLeftShort size={28} className="text-gray-600 shrink-0" />
                        <span className="text-md font-[450] text-gray-800 whitespace-nowrap">Back to Store</span>
                    </div>
                </Link>
                ...
                <Ellipsis className={`text-gray-500 ml-4 transition-all duration-300 ${shouldExpand ? 'opacity-0 translate-x-0 translate-y-5' : 'opacity-100 translate-x-2 -translate-y-6'}`} />

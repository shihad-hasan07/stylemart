...
import ProductPreviewDetails from '@/app/dashboard/components/products_componetns/DemoProducts_preview';
...
const add_products = () => {
    ...
    return (
        <div>
            <Routes_heading_texts name={'add product'} backButtonLink={true} isSubmitting={isSubmitting} />

            {openPreview ?
                <div onClick={handleClose} className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
                    <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-[95vw] sm:max-w-[90vw] max-h-[95vh] sm:max-h-[90vh] overflow-y-auto 
                        px-4 sm:px-6 lg:px-10 rounded-xl relative">
                ...
                >
                    ...
                    <ProductVariations variations={productVaritaions} setProductVaritaions={setProductVaritaions} dropDown={{ openDropdown, setOpenDropdown }} />

                    <div className='flex flex-col sm:flex-row gap-4 items-center justify-end mt-6'>
                        <button type="button" disabled={isSubmitting} onClick={() => setOpenPreview(true)} className="cursor-pointer w-full sm:w-auto px-6 py-2.5 rounded-lg border-2 border-gray-300 bg-white text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                            Preview
                        </button>

                        <button type='submit' disabled={isSubmitting} className="cursor-pointer w-full sm:w-auto px-6 py-2.5 rounded-lg font-semibold bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
